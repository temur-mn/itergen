...
- `itergen.api`: high-level runtime facade (`synthesizer`) and public dataclasses (`models`).
- `itergen.schema`: config parsing/validation (`config`), defaults (`defaults`), and built-in samples (`samples`).
- `itergen.engine`: initial generation (`initial`), retry loop (`generation`), optimizer (`optimizer`), and proposal helpers (`adjustments`).
- `itergen.scoring`: condition matching (`conditions`) and equilibrium/objective reporting (`metrics`).
- `itergen.controllers`: classic controller (`classic`) and torch-backed controller (`torch`).
- `itergen.runtime`: deterministic RNG helpers (`rng`) and run logging (`logging_utils`).
- Legacy flat-module imports (for example `itergen.config`, `itergen.metrics`)
  have been removed; import from canonical packages such as `itergen.api`,
  `itergen.schema`, `itergen.engine`, and `itergen.scoring`.
...
- Generated dataset workbook (`output/<timestamp>_itergen.xlsx` by default).
- Per-run log file under configured `log_dir` (default `./logs/`).
