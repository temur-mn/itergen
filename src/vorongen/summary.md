...
- `attempt_workers`: process workers for parallel retry attempts (default `1`).
...
Use `project/benchmarks/benchmark_attempt_workers.py` to compare wall-time impact of
parallel retry attempts (`attempt_workers`).

Example:

- `python -m project.benchmarks.benchmark_optimizer_scoring`
- `python -m project.benchmarks.benchmark_attempt_workers`
...
- Attempt-worker benchmark also supports: `BENCH_MAX_ATTEMPTS`, `BENCH_WORKERS`.
